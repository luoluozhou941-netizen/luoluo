...
async function generateSegmentSummary(segment,messages,opts){
  ...
  const body={
    ...
    messages:[
      ...
      {role:'user',content:`以下是这段话题的聊天记录：\n\n${transcript}${hintLine}\n\n请按约定格式输出JSON。`}
    ],
    stream:false
  };
  try{
...
function renderProviderForm(p){
  ...
  form.querySelector('[data-act="save"]').onclick=()=>{
    ...
    const data={id:formData.id,name,baseUrl,apiKey,defaultModel,models:formData.models||[],reasoningEffort,note,createdAt:p?p.createdAt:Date.now()};
...
function openSettings(){
  ...
  const html=`<h3><i class="ph-light ph-gear"></i> 房间设置</h3>
    ...
    <input id="s-inject-days" type="number" min="1" max="30" value="${settings.dailyInjectDays}">
  ...
  showModal(html,()=>{
    ...
    settings.dailyInjectDays=Math.max(1,Math.min(30,parseInt(document.getElementById('s-inject-days').value)||7));
